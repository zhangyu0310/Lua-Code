...
static void print_object(const TObject* v) {
  print_space(' ');
  switch (ttype(v)) {
  case LUA_T_USERDATA:
    printf("user data: [%p]\n", v->value.ts->u.d.v);
...
static void luaB_printtable(void) {
  lua_Object o = lua_getparam(1);
  if (lua_istable(o)) {
    table_count++;
    TObject* obj = luaA_Address(o);
    Hash* h = obj->value.a;
    print_table_info(h);
    for (int i = 0; i < nhash(h); ++i) {
      if (ttype(ref(node(h, i))) != LUA_T_NIL) {
        TObject* v = val(node(h, i));
        print_object(v);
      }
    }
    table_count--;
...
static void luaB_listtables(void) {
  GCnode* node = L->rootglobal.next;
  while (node != NULL) {
    TaggedString* ts = (TaggedString*)node;
    printf("%s\n", ts->str);
    node = node->next;
  }
}
