...
static void print_space(char c) {
  for (int i = 0; i < (table_count - 1) * 2; ++i) {
    printf("%c", c);
  }
  printf(" ");
}

static void print_table_info(const Hash* h) {
  print_space(' ');
  printf("Table %d: [%p]\n", table_count, h);
  printf("------------------------------\n");
}

static void print_object(lua_State* L, const TObject* v, const char* prefix) {
  print_space(' ');
  printf("%s: ", prefix);
  switch (ttype(v)) {
  case LUA_TUSERDATA:
...
static int luaB_printtable (lua_State *L) {
  Hash* h = lua_topointer(L, 1);
  if (h == NULL) {
    return 0;
  }
  table_count++;
  ...
  for (int i = 0; i < h->size; ++i) {
    node = h->node + i;
    if (ttype(&(node->key)) == LUA_TNIL)
      continue;
    print_object(L, &(node->key), " Key");
    print_object(L, &(node->val), " Value");
    printf("------------------------------\n");
  }
  table_count--;
  return 0;
...
static int luaB_listtables (lua_State* L) {
  int n = lua_gettop(L);
  lua_getglobals(L);
  struct Hash* h = lua_topointer(L, n + 1);
  if (h == NULL) return 0;
  Node* node = h->node;
  for (int i = 0; i < h->size; ++i) {
    char name[1024] = { 0 };
    node = h->node + i;
    switch (ttype(&(node->key))) {
    case LUA_TNUMBER:
      itoa(nvalue(&(node->key)), name, 10);
      break;
    case LUA_TSTRING:
      strncpy(name, svalue(&(node->key)), 1024);
      break;
    default:
      break;
    }
    if (ttype(&(node->val)) == LUA_TTABLE) {
      printf("Table name: %s\n", name);
      if (n > 0) {
        lua_getglobal(L, "printtable");
        lua_pushvalue(L, -1);
...
static const struct luaL_reg base_funcs[] = {
  ...
  {"printtable", luaB_printtable},
  {"listtables", luaB_listtables},
};
